...
export function PermissionBadge({
  ...
  return (
    <span
      className={`permission-badge ${risk.tone}`}
      title={
        permissions.length ? permissions.join(", ") : "No permissions declared"
      }
    >
...
type CardProps = { plugin: Plugin; locale: Locale; featured?: boolean };

function CardHeading({ plugin, locale }: CardProps) {
  const version = currentVersion(plugin);
  const primaryCategory =
    plugin.categories.find((category) => category !== "official") ??
    plugin.categories[0];
  ...
  return (
    <div className="plugin-card-topline">
      <div className="plugin-icon">
        <PluginIcon id={plugin.id} category={primaryCategory} />
      </div>
      <div className="plugin-card-meta">
        {plugin.verified && (
          <span className="verified-label">
            <Check size={13} /> {officialLabel}
          </span>
        )}
...
export function PluginCard({ plugin, locale, featured = false }: CardProps) {
  const localized = localizedPlugin(plugin, locale);
  const version = currentVersion(plugin);
  return (
    <article className={`plugin-card ${featured ? "featured-card" : ""}`}>
      <CardHeading plugin={plugin} locale={locale} />
      ...
      <div className="plugin-card-footer">
        ...
        <a
          ...
          aria-label={`${getCopy(locale).detail.source}: ${localized.name}`}
        >
